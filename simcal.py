...
holiday_lang = "ja"  # 'en': English, 'ja': Japanese, etc. (ISO 639-1)
...
for m, month in enumerate(months, start=1):
    ...
    for i, day in enumerate(week_days):
        ...
        if i != 0:
            canvas.line(24 + i * cell_width, height - 108, 24 + i * cell_width, 32)
            ...
            if date:
                ...
                if holiday_region:
                    ...
                    holiday = holidays.country_holidays(
                        holiday_region, language=holiday_lang
                    ).get(datetime.date(year, m, date))
                if holiday:
                    holiday = holiday[:24]
                    canvas.drawString(
                        28 + i * cell_width, height - 148 - cell_height * w, holiday
                    )
                canvas.setFont(efont, 26)
                if wd[i] == "Saturday":
                    canvas.setFillColorRGB(0, 0, 1)  # blue
                if wd[i] == "Sunday" or holiday:
                    canvas.setFillColorRGB(1, 0, 0)  # red
                canvas.drawString(
                    28 + i * cell_width, height - 132 - cell_height * w, str(date)
                )
                canvas.setFillColorRGB(0, 0, 0)
